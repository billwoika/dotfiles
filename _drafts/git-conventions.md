...
> **Status: Draft.** This document is a starter for the future engineering
> handbook. The patterns here are *proposals* — opinions about how a team
> might work with git that the framework deliberately does not enforce.
>
> The framework (Section 17 of the Developer Environment Framework)
> covers what *can* be enforced by tools: pre-commit hooks, branch
> protection, lockfile diff suppression, conventional commit hook
> validation. This document covers what can only be adopted as norms:
> commit message style, PR conventions, branching strategy, code review
> etiquette.
>
...
- **Always include the Ticket Number in the subject.** The ticket 
  The full body of the commit is rarely read in practice, using the
  subject line allows for quick grep'ing of log outputs to isolate 
  the linkage between tickets and commits.
...
- **Wrap at 72 characters.** `git log` formats commit bodies in a
  ...
  framework's vim configuration (Section 22.2.1) does this automatically
  for `gitcommit` filetype.
...
If adopting, the framework can enforce the format via lefthook
(Section 17.10.3).
...
- **Link PRs from different repos**. If the requirements dictate
  cross-repo modification, each PR should contain a reference to the
...
- **Pushed to the wrong branch?** Force-push the original state back
  ...
  `gpushf` alias (Section 17.8) does this safely.
...
- **Specific Vendor Issues workflow integration.** Each
...
This is a draft, kept in the dotfiles repo's `_drafts/` directory as
starter content for the future engineering handbook. It's deliberately
not part of the framework's published documentation site.

When the team adopts these conventions (or some modified version),
this document should:

1. Move into the engineering handbook repo
2. Be edited by the team to reflect actual decisions
3. Drop the "Status: Draft" framing and become normative for that team
4. Get cross-referenced from the framework's git chapter, not embedded
   in it

The framework's chapter (Section 17) and this document have different
purposes and different lifecycles. Don't merge them.
