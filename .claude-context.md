...
├── README.md                Layer-1 short-form intro (13 KB)
...
│   ├── git-conventions.md   Future engineering handbook stub
│   └── README-v2.6-old.md   Previous README, archived
├── archive/                 Word manuscript snapshots
│   ├── README.md
│   └── v2.7-snapshot.docx   209-page frozen manuscript
├── docs/                    MkDocs source
│   ├── index.md
│   ├── requirements.txt     Pinned MkDocs dependencies
│   ├── shell/               (index + architecture)
│   ├── tools/               (index + mise-and-make)
│   ├── git/                 (index + tool-enforced-practices)
│   ├── operations/          (index + secrets)
│   ├── reference/           (index + customization)
│   └── handbook/            Opinion-heavy companion section
│       ├── index.md
│       └── design/
│           ├── index.md
│           ├── separation-of-concerns.md     (stub)
│           ├── locality.md                   (stub)
│           ├── separation-by-lifecycle.md    (stub)
│           ├── separation-by-dependency.md   (stub)
│           ├── separation-by-boundary.md     (stub)
│           ├── separation-by-variation.md    (stub)
│           └── value-types.md                (stub)
...
├── jetbrains/               README + 2 runConfiguration XMLs
├── macos/                   File-association setup script
├── devcontainer/            5 reference templates
└── sh/tests/profile_test.sh POSIX test suite (30 tests)
```

**File counts:** 81 regular files, 28 directories. **Tarball:** ~324 KB
including the archived Word doc; ~95 KB without it.

**Build state at the time of writing:**

- `mkdocs build --strict` passes in ~1 second, 20 pages generated
- 22 zsh files pass `zsh -n`
- 6 POSIX files pass `sh -n`
- POSIX test suite passes 30/30
- All JSON config templates parse
- All XML run configurations parse
...
### 10.3 IDE migration (about to happen)

Bill's stated next step (at the time this document was written) is
to move the conversation from chat to an IDE workflow. The
documents are at the size where round-tripping content edits through
chat is friction; an IDE lets him read the rendered MkDocs site
alongside the source while he iterates.

This document is the primary handoff artifact for that move.

### 10.4 Repository reorganization

Bill mentioned wanting to "re-organize the layout when I move to the
IDE." The current layout (Section 2) is functional but not
permanent. Future-Claude: the layout in this document reflects the
state at the time of writing; the actual repo may have moved files
around since then.
...
*End of `.claude-context.md`. ~7800 words.*
